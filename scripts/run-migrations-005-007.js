...
// ── Supabase REST API method (fallback) ─────────────────────────────────────
...
async function runViaRest() {
  ...
  for (const migrationFile of MIGRATIONS) {
    ...
    for (const stmt of statements) {
      ...
      process.stdout.write(`  ${preview}... `)
      try {
        // Use Supabase's SQL endpoint via REST
        ...
        if (response.ok) {
          ...
          if (body.includes('already exists') || body.includes('does not exist')) {
            ...
            // exec_sql function doesn't exist — try pg-meta endpoint
            throw new Error('exec_sql RPC not available')
          } else {
            console.log(`WARN: ${body.slice(0, 120)}`)
            success++ // Continue — idempotent migrations are safe
          }
        }
      } catch (err) {
...
// ── Supabase Management API method ──────────────────────────────────────────
