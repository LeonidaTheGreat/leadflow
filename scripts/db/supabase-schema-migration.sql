-- Supabase Schema Migration: Move DASHBOARD.md sections to database
-- This makes Supabase the single source of truth for ALL dashboard data
-- 
-- Tables to create:
-- 1. project_metadata - Project info (goal, deadline, status)
-- 2. system_components - System status (Vercel, FUB, Twilio, etc.)
-- 3. agents - Agent activity tracking
-- 4. completed_work - Completed features/tasks
-- 5. action_items - Blockers and outstanding items
-- 6. cost_tracking - Budget and cost summaries
...
CREATE TABLE IF NOT EXISTS project_metadata (
  ...
  overall_status VARCHAR(50), -- ACTIVE, PAUSED, COMPLETE, AT_RISK
  status_color VARCHAR(20), -- 🟢 GREEN, 🟡 YELLOW, 🔴 RED
  last_updated TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  metadata JSONB,
  
  CONSTRAINT valid_status CHECK (overall_status IN ('ACTIVE', 'PAUSED', 'COMPLETE', 'AT_RISK'))
);

-- ==================== SYSTEM COMPONENTS ====================
CREATE TABLE IF NOT EXISTS system_components (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id VARCHAR(50) NOT NULL REFERENCES project_metadata(project_id),
  component_name VARCHAR(100) NOT NULL,
  category VARCHAR(50), -- DEPLOYMENT, INTEGRATION, DATABASE, COMPLIANCE, TESTING
  status VARCHAR(50), -- LIVE, READY, TESTING, DOWN, PENDING
  status_emoji VARCHAR(10), -- ✅, 🟡, ❌, ⏳, etc.
  details VARCHAR(500),
...
CREATE TABLE IF NOT EXISTS agents (
  ...
  agent_type VARCHAR(50), -- dev, marketing, qc, analytics, product, etc.
  status VARCHAR(50), -- ACTIVE, READY, WAITING, COMPLETE, BLOCKED
...
CREATE TABLE IF NOT EXISTS completed_work (
  ...
  use_case VARCHAR(50), -- UC-1, UC-2, etc. or null
  description TEXT,
  category VARCHAR(100), -- FEATURE, DEPLOYMENT, INTEGRATION, TESTING, etc.
  hours_spent NUMERIC(8,2),
  completed_date TIMESTAMP,
  status VARCHAR(50), -- COMPLETE, IN_PROGRESS, BLOCKED
  dependencies JSONB, -- Array of task IDs this unblocks
  metadata JSONB,
...
CREATE TABLE IF NOT EXISTS action_items (
  ...
  type VARCHAR(50), -- BLOCKER, DECISION, APPROVAL, FOLLOW_UP, RISK
  status VARCHAR(50), -- OPEN, IN_PROGRESS, RESOLVED, WAITING
  priority INT DEFAULT 2, -- 1=critical, 2=high, 3=normal, 4=low
  description TEXT,
  assigned_to VARCHAR(100),
  awaiting_input VARCHAR(100), -- Who needs to decide
  impact VARCHAR(255), -- Business impact
  action_needed TEXT,
  due_date TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS cost_tracking (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id VARCHAR(50) NOT NULL REFERENCES project_metadata(project_id),
  budget_period VARCHAR(50), -- TOTAL, DAILY, WEEKLY, MONTHLY
  estimated_cost_usd NUMERIC(12,2),
  actual_cost_usd NUMERIC(12,2),
  budget_limit_usd NUMERIC(12,2),
  spend_percent NUMERIC(5,2), -- 0-100%
  breakdown JSONB, -- { "sonnet": 45.50, "haiku": 12.30, ... }
...
CREATE TABLE IF NOT EXISTS dashboard_snapshots (
  ...
  -- Full snapshot as JSON for archival
  full_snapshot JSONB,
