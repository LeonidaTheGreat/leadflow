...
What:
- Add scripts/no-direct-db.js with a route-layer guard that scans backend route files and fails if database-style `.from()` calls are present.
...
- Run `node ~/.openclaw/genome/scripts/quality-audit.js . --json` and confirm the current no_direct_db failure source is dashboard `app/api` files, while backend routes remain clean.
