...
- git log --oneline --follow -- product/lead-response/dashboard/scripts/cleanup-next-build-lock.js
  confirms 4+ subsequent merges on main after the orphan branch date.
...
the **identical message** as branch tip `44e0f83b`, confirming the fix was cherry-picked
or re-implemented rather than merged from this branch.

The affected file was then evolved through 4 subsequent merged PRs:
...
**Low.** Single file, fully superseded by 4 later PRs on main. No unique code survives.
...
git log --format="%H %s %ad" --date=short origin/dev/33f99fdf-fix-quality-gate-build-failing-in-leadfl ^main
git log --oneline --follow -- product/lead-response/dashboard/scripts/cleanup-next-build-lock.js
git show f4aa4e20 --stat
git show 62b7dc85 --stat
git show 6114e9d5 --stat
psql $LOCAL_PG_URL -c "SELECT ... FROM tasks WHERE branch_name LIKE '%33f99fdf%'"
```
