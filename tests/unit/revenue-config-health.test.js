...
test('returns camelCase response fields (secretKey not secret_key)', () => {
  const src = fs.readFileSync(
    path.join(DASHBOARD, 'app/api/admin/revenue-config-health/route.ts'), 'utf8')
  assert.ok(src.includes('secretKey'), 'should use camelCase secretKey')
  assert.ok(!src.includes('secret_key:'), 'should not use snake_case secret_key')
})
...
test('create-checkout-session uses PRICE_ENV_MAP (not PLAN_ENV_MAP)', () => {
  const src = fs.readFileSync(
    path.join(DASHBOARD, 'app/api/billing/create-checkout-session/route.ts'), 'utf8')
  assert.ok(src.includes('PRICE_ENV_MAP'), 'should use PRICE_ENV_MAP')
  assert.ok(!src.includes('PLAN_ENV_MAP'), 'should not use old PLAN_ENV_MAP')
})

test('create-checkout-session uses getPriceIdForPlan helper', () => {
  const src = fs.readFileSync(
    path.join(DASHBOARD, 'app/api/billing/create-checkout-session/route.ts'), 'utf8')
  assert.ok(src.includes('getPriceIdForPlan'), 'should have getPriceIdForPlan helper')
})
...
test('upgrade-checkout uses PRICE_ENV_MAP (not PLAN_ENV_MAP)', () => {
  const src = fs.readFileSync(
...
test('upgrade-checkout returns PRICE_NOT_CONFIGURED', () => {
  const src = fs.readFileSync(
    path.join(DASHBOARD, 'app/api/stripe/upgrade-checkout/route.ts'), 'utf8')
...
test('activation page renders RevenueConfigBanner', () => {
  const src = fs.readFileSync(
...
test('activation page checks camelCase response fields', () => {
  const src = fs.readFileSync(
    path.join(DASHBOARD, 'app/admin/activation/page.tsx'), 'utf8')
  assert.ok(src.includes('secretKey'), 'should check secretKey (camelCase)')
  assert.ok(src.includes('webhookSecret'), 'should check webhookSecret')
  assert.ok(src.includes('resendApiKey'), 'should check resendApiKey (camelCase)')
})
