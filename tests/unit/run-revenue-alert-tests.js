#!/usr/bin/env node
/**
 * Simple test runner for revenue alert idempotency tests
...
async function runTests() {
  console.log('\n🧪 Revenue Alert Idempotency Tests\n')

  await test('AC1: Creates first alert for critical MRR goal', async () => {
  ...
  await test('AC2: Skips duplicate alerts within 48h window', async () => {
    const store = new MockTaskStore()
  ...
  await test('AC3: Creates new task when trajectory changes', async () => {
    const store = new MockTaskStore()
  ...
  await test('AC4: Skips on-track goals', async () => {
    const store = new MockTaskStore()
  ...
  await test('AC5: Handles multiple goals independently', async () => {
    const store = new MockTaskStore()
    ...
    assert.strictEqual(result.created, 2, 'Should create 2 tasks')
    assert.strictEqual(store.tasks.length, 2, 'TaskStore should have 2 tasks')
  ...
  await test('AC6: Preserves dedup_key for loop detection', async () => {
    const store = new MockTaskStore()
  ...
  await test('AC7: Summary statistics are accurate', async () => {
    const store = new MockTaskStore()
