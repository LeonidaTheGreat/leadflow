...
// ─── Webhook route: 503 when secret missing ───────────────────────────────────

console.log('\n=== routes/billing.js: 503 guard for missing STRIPE_WEBHOOK_SECRET ===\n');

check('billing route rejects with 503 when STRIPE_WEBHOOK_SECRET is missing', () => {
  const billingPath = path.join(__dirname, '..', '..', 'routes', 'billing.js');
  const src = fs.readFileSync(billingPath, 'utf8');
  assert.ok(src.includes('STRIPE_WEBHOOK_SECRET'),
    'billing.js does not reference STRIPE_WEBHOOK_SECRET');
  assert.ok(src.match(/status\(503\)/),
    'billing.js does not return 503 when webhook secret is missing');
});
