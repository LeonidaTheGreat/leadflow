/**
 * E2E test: send-payment-link-email API route and payment-link tier support
 * Tests auth gate, input validation, tier validation, and 404 for unknown agents.
...
async function run() {
  ...
  await test('Missing agentId returns 400 or 503', async () => {
  ...
  await test('payment-link route: invalid tier returns 400 or 503', async () => {
  ...
  await test('payment-link route: missing agentId and agentEmail returns 400', async () => {
    const res = await request('POST', PAYMENT_LINK_ENDPOINT, { tier: 'starter' }, cookie)
    assert.ok([400, 503].includes(res.status), `Expected 400 or 503, got ${res.status}`)
  })
  ...
  await test('payment-link route: pro tier accepted', async () => {
    const res = await request('POST', PAYMENT_LINK_ENDPOINT, { agentEmail: 'test@test.com', tier: 'pro' }, cookie)
    assert.ok([200, 503].includes(res.status), `Expected 200 or 503, got ${res.status}`)
  })

  await test('payment-link route: team tier accepted', async () => {
    const res = await request('POST', PAYMENT_LINK_ENDPOINT, { agentEmail: 'test@test.com', tier: 'team' }, cookie)
    assert.ok([200, 503].includes(res.status), `Expected 200 or 503, got ${res.status}`)
  })
