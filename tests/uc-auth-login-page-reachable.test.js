...
async function runTests() {
  ...
  try {
    ...
    // Parse non-zero text insertions: match "N insertion" where N > 0
    ...
    // Check if only binary files changed
    const hasBinaryFiles = commitStat.includes('Bin ')
    const hasTextChanges = textInsertions > 0 || textDeletions > 0
    const hasLayoutFiles = /\.(tsx|ts|css|jsx|js)\b/.test(commitStat) &&
      (commitStat.includes('login') || commitStat.includes('signup') ||
       commitStat.includes('global') || commitStat.includes('tailwind') ||
       commitStat.includes('layout') || commitStat.includes('page'))

    if (hasBinaryFiles && !hasTextChanges) {
      fail(
        'PR #1049 dev commit contains layout source changes',
        `Commit ${PR_1049_COMMIT.slice(0,8)} is binary-only (trace.zip, 0 text insertions/deletions). ` +
    ...
    } else if (hasTextChanges && !hasLayoutFiles) {
      fail(
        'PR #1049 dev commit modifies login/signup layout files',
        `Commit ${PR_1049_COMMIT.slice(0,8)} has text changes but not in login/signup layout source files.`
      )
    } else if (hasTextChanges && hasLayoutFiles) {
      pass(`PR #1049 dev commit modifies layout source files (${textInsertions} insertions, ${textDeletions} deletions)`)
    } else {
      fail(
        'PR #1049 dev commit contains any source changes',
        `Commit ${PR_1049_COMMIT.slice(0,8)}: no text changes detected.`
      )
    }
  } catch (e) {
