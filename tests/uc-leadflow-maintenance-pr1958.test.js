...
// ── Test 6: no contradiction — noCodeChanges claim vs actual diff ─────────────
test('verdict JSON noCodeChanges claim is false (package.json IS changed)', () => {
  const verdictPath = path.join(PROJECT_DIR, 'docs/reports/orphan-branch-098f80d4-verdict.json');
  if (!fs.existsSync(verdictPath)) return;
  const doc = JSON.parse(fs.readFileSync(verdictPath, 'utf8'));
  ...
  const hasTestSuiteFloor = 'testSuiteFloor' in pkg;
  // If the verdict claims no code changes but package.json has testSuiteFloor, that is contradictory.
  // This test documents the inconsistency so QC can flag it.
  if (claimsNoCodeChanges && hasTestSuiteFloor) {
    assert.fail(
      'Contradiction: verdict JSON claims noCodeChanges=true, but package.json contains testSuiteFloor field'
    );
  }
});
