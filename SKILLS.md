...
### SKILL 12: replenish_queue_from_roadmap
**Purpose:** When queue < 2 ready tasks, read `use_cases` table and create the next workflow step
**Automated:** Yes, in `replenishQueue()` every heartbeat
**Manual trigger:** When you notice the queue is empty and want to understand why
```javascript
// Queries use_cases table for incomplete UCs ordered by priority
// Checks dependency resolution (depends_on array)
// Creates task for first workflow step (usually PM spec)
// Maps agent IDs: product=PM, dev=Dev, design=Design, qc=QC, analytics=Analytics
```

### SKILL 13: chain_workflow_steps
**Purpose:** When a task completes, create the next step in the UC workflow
**Automated:** Yes, in `createFollowUpTasks()` called from `handleTaskSuccess()`
**Example chain:** PM spec -> Dev implementation -> QC review -> UC marked complete
```javascript
// Reads use_cases.workflow array (e.g., ['product', 'dev', 'qc'])
// Finds current agent's position in workflow
// Creates task for next agent
// On last step: marks UC as implementation_status='complete'
```

### SKILL 14: process_product_feedback
**Purpose:** Route unprocessed feedback to PM agent for analysis
**Automated:** Yes, in `processProductFeedback()` every heartbeat (max 3/cycle)
**Feedback sources:** E2E test failures, PostHog funnels, pilot users
**Manual:** Insert into `product_feedback` table for anything that needs PM attention

### SKILL 15: manage_pr_reviews
**Purpose:** Auto-merge approved PRs, create fix tasks for rejected ones
**Automated:** Yes, in `checkPRReviews()` every heartbeat
**Manual:** Check `code_reviews` table for pending reviews

### SKILL 16: branch_workflow
**Purpose:** Create feature branches for dev/design agents
**Automated:** Yes, in `spawn-consumer.js` when spawning dev or design agents
**Branch format:** `{agent}/{taskid-8chars}-{sanitized-title}`
