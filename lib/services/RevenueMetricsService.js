...
class RevenueMetricsService {
  ...
  async computeSnapshot(snapshotDate = new Date()) {
    // Guard: exclude test subscription IDs (sub_test_*) from all MRR calculations.
    // Test subscriptions are inserted by integration tests and must never pollute revenue metrics.
    const EXCLUDE_TEST_SUBS = `stripe_subscription_id NOT LIKE 'sub_test_%'`;

    const activeSubscribers = await this.scalar(`SELECT COUNT(*)::int FROM subscriptions WHERE status = 'active' AND ${EXCLUDE_TEST_SUBS}`);
    ...
    const mrrCents = await this.scalar(`SELECT COALESCE(SUM(CASE plan_tier WHEN 'starter' THEN 4900 WHEN 'pro' THEN 14900 WHEN 'team' THEN 39900 WHEN 'brokerage' THEN 99900 ELSE 0 END),0)::int FROM subscriptions WHERE status = 'active' AND ${EXCLUDE_TEST_SUBS}`);

    const newSubscribers = await this.scalar(
      `SELECT COUNT(*)::int FROM subscriptions WHERE status = 'active' AND ${EXCLUDE_TEST_SUBS} AND activated_at >= $1::date AND activated_at < ($1::date + INTERVAL '1 day')`,
