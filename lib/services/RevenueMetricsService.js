...
class RevenueMetricsService {
  ...
  async computeSnapshot(snapshotDate = new Date()) {
    // Exclude test subscription IDs (sub_test_*) from all revenue metrics to prevent test data polluting MRR
    const REAL_SUB_FILTER = "stripe_subscription_id NOT LIKE 'sub_test_%'";
    const activeSubscribers = await this.scalar(`SELECT COUNT(*)::int FROM subscriptions WHERE status = 'active' AND ${REAL_SUB_FILTER}`);
    ...
    const mrrCents = await this.scalar(`SELECT COALESCE(SUM(CASE plan_tier WHEN 'starter' THEN 4900 WHEN 'pro' THEN 14900 WHEN 'team' THEN 39900 WHEN 'brokerage' THEN 99900 ELSE 0 END),0)::int FROM subscriptions WHERE status = 'active' AND ${REAL_SUB_FILTER}`);

    const newSubscribers = await this.scalar(
      `SELECT COUNT(*)::int FROM subscriptions WHERE status = 'active' AND ${REAL_SUB_FILTER} AND activated_at >= $1::date AND activated_at < ($1::date + INTERVAL '1 day')`,
