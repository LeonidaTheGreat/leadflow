...
class RevenueMetricsService {
  ...
  async computeSnapshot(snapshotDate = new Date()) {
    ...
    const mrrCents = await this.scalar(`SELECT COALESCE(SUM(CASE tier WHEN 'starter' THEN 4900 WHEN 'pro' THEN 14900 WHEN 'professional' THEN 14900 WHEN 'team' THEN 39900 WHEN 'brokerage' THEN 99900 WHEN 'enterprise' THEN 99900 ELSE 0 END),0)::int FROM subscriptions WHERE status = 'active'`);

    const newSubscribers = await this.scalar(
      `SELECT COUNT(*)::int FROM subscriptions WHERE status = 'active' AND created_at >= $1::date AND created_at < ($1::date + INTERVAL '1 day')`,
