    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (error.code === 'PGRST116' || // Not found
                ...
                log.warn(`${operationName} failed with non-retryable error`, { error: error.message, code: error.code });
                throw error;
            }
            
            if (attempt < maxRetries) {
                const delay = calculateBackoffDelay(attempt);
                log.warn(`${operationName} failed, retrying`, { attempt: attempt + 1, maxAttempts: maxRetries + 1, delayMs: delay, error: error.message });
                await sleep(delay);
            } else {
                log.warn(`${operationName} failed after all attempts`, { attempts: maxRetries + 1, error: error.message });
            }
        }
    }
...
function verifyWebhookSignature(payload, signature) {
    ...
    if (!secret) {
        log.warn('CAL_WEBHOOK_SECRET not configured - skipping signature verification');
...
async function handleCalWebhook(event) {
    ...
    try {
        ...
        log.error('Error handling Cal.com webhook', error, { eventType });
        throw error;
    }
}
...
async function handleBookingCreated(booking) {
    ...
    log.info('Booking attendee details', { name: attendee.name, email: attendee.email, startTime: booking.startTime, eventType: booking.eventType?.slug || booking.eventTypeId });
