...
async function sendBookingConfirmationSMS(bookingData) {
    if (!bookingData.phone) {
        console.warn('sendBookingConfirmationSMS: no phone number provided, skipping');
        return;
    }
    ...
    console.info(`SMS booking confirmation sent to ***${String(bookingData.phone).slice(-4)}`);
}
...
async function sendRescheduleConfirmationSMS(bookingData) {
    if (!bookingData.phone) {
        console.warn('sendRescheduleConfirmationSMS: no phone number provided, skipping');
        return;
    }
    ...
    console.info(`SMS reschedule confirmation sent to ***${String(bookingData.phone).slice(-4)}`);
}
