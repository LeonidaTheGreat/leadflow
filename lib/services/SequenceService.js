...
class SequenceService {
  ...
  async findLeadByFubId(fubId) {
    ...
    if (error || !data) {
      log.warn(`⚠️  Could not find lead for fub_id=${fubId}: ${error?.message}`);
      return null;
    }
    return data.id;
  }
  ...
  async findLeadByPhone(phone) {
    ...
    if (error || !data) {
      log.warn(`⚠️  Could not find lead for phone=${phone}: ${error?.message}`);
      return null;
    }
    return data.id;
  }
  ...
  async hasActiveSequence(leadId, sequenceType) {
    ...
    if (error) {
      log.error(`❌ Error checking existing sequences: ${error.message}`, error);
      return false;
    }
  ...
  async createLeadSequence(params) {
    ...
    if (!VALID_SEQUENCE_TYPES.includes(sequence_type)) {
      log.error(`❌ createLeadSequence: invalid sequence_type "${sequence_type}"`, null);
      return null;
    }
    ...
    if (alreadyActive) {
      log.info(`ℹ️  Active "${sequence_type}" sequence already exists for lead ${lead_id}, skipping`);
      return null;
    }
    ...
    if (error) {
      log.error(`❌ Error creating "${sequence_type}" sequence for lead ${lead_id}:`, error);
      return null;
    }

    log.info(`✅ Created "${sequence_type}" sequence for lead ${lead_id} (next_send_at: ${sendAt})`);
    return data;
  }
}
