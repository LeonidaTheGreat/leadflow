...
const log = logger.child('SequenceService');
...
class SequenceService {
  ...
  async findLeadByFubId(fubId) {
    ...
    if (error || !data) {
      log.warn('Could not find lead by fub_id', { fubId, error: error?.message });
      return null;
    }
    return data.id;
  }
  ...
  async findLeadByPhone(phone) {
    ...
    if (error || !data) {
      log.warn('Could not find lead by phone', { phone, error: error?.message });
      return null;
    }
    return data.id;
  }
  ...
  async hasActiveSequence(leadId, sequenceType) {
    ...
    if (error) {
      log.error('Error checking existing sequences', error, { leadId, sequenceType });
      return false;
    }
  ...
  async createLeadSequence(params) {
    ...
    if (!lead_id) {
      log.error('createLeadSequence: lead_id is required', null);
      return null;
    }

    if (!VALID_SEQUENCE_TYPES.includes(sequence_type)) {
      log.error('createLeadSequence: invalid sequence_type', null, { sequence_type });
      return null;
    }

    const db = this._getDb();
    if (!db) {
      log.error('createLeadSequence: DB client not available', null);
      return null;
    }

    const alreadyActive = await this.hasActiveSequence(lead_id, sequence_type);
    if (alreadyActive) {
      log.info('Active sequence already exists for lead, skipping', { sequence_type, lead_id });
      return null;
    }
    ...
    if (error) {
      log.error('Error creating sequence for lead', error, { sequence_type, lead_id });
      return null;
    }

    log.info('Created sequence for lead', { sequence_type, lead_id, next_send_at: sendAt });
    return data;
  }
}
