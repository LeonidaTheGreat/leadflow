...
class CalcomService {
    constructor(options = {}) {
        this._apiKey = 'apiKey' in options ? options.apiKey : undefined;
        this._webhookSecret = 'webhookSecret' in options ? options.webhookSecret : undefined;
        this._username = 'username' in options ? options.username : undefined;
    ...
    get apiKey() {
        return this._apiKey !== undefined ? this._apiKey : (process.env.CAL_API_KEY || null);
    }

    get webhookSecret() {
        return this._webhookSecret !== undefined ? this._webhookSecret : (process.env.CAL_WEBHOOK_SECRET || null);
    }

    get username() {
        return this._username !== undefined ? this._username : (process.env.CAL_USERNAME || null);
    }
    ...
    async getEventTypes(filters = {}) {
        ...
        try {
            ...
            if (process.env.NODE_ENV !== 'production') return this._mockEventTypes();
            throw error;
        }
    }
    ...
    async getEventType(eventTypeId) {
        if (!this.isConfigured()) {
            const mock = this._mockEventTypes().find(et => et.id === eventTypeId);
            return mock || this._mockEventTypes()[0];
        }
    ...
    async getBooking(bookingId) {
        if (!this.isConfigured()) return this._mockBooking({ id: bookingId });
    ...
    async handleBookingCancelled(booking) {
        ...
        try {
            ...
            if (existingBooking?.lead_id) {
                try {
                    await sequenceService.createLeadSequence({ lead_id: existingBooking.lead_id, sequence_type: 'no_show', trigger_reason: 'missed_appointment', metadata: { cal_booking_id: existingBooking.id, cal_booking_uid: booking.uid, cancellation_reason: booking.cancellationReason || null, triggered_by: 'BOOKING_CANCELLED' } });
                } catch (seqError) {
    ...
    async _triggerPostMeetingFollowUp(booking) {
        ...
        await sequenceService.createLeadSequence({ lead_id: booking.lead_id, sequence_type: 'post_viewing', trigger_reason: 'meeting_ended', metadata: { cal_booking_id: booking.id, cal_booking_uid: booking.cal_booking_uid, triggered_by: 'MEETING_ENDED' } });
    }
    ...
    async listWebhooks() {
        const db = this._getDb();
        if (!db) return this._mockWebhooks();
        try {
            ...
            if (process.env.NODE_ENV !== 'production') return this._mockWebhooks();
            throw error;
        }
    }
    ...
    async getWebhook(webhookId) {
        const db = this._getDb();
        if (!db) {
            const mock = this._mockWebhooks().find(w => w.webhookId === webhookId);
    ...
    async getWebhookDeliveryLogs(filters = {}) {
        const db = this._getDb();
        if (!db) return this._mockDeliveryLogs();
        try {
            ...
            if (process.env.NODE_ENV !== 'production') return this._mockDeliveryLogs();
            throw error;
        }
    }
    ...
    async getWebhookStats(webhookId, dateRange = {}) {
        ...
        try {
            ...
            if (process.env.NODE_ENV !== 'production') return this._mockStats();
            throw error;
        }
    }
    ...
    /**
     * Persist the Cal.com booking URL for an agent.
     * @param {string} agentId
     * @param {string} bookingUrl
     * @returns {Promise<Object|null>}
     */
    async storeAgentBookingUrl(agentId, bookingUrl) {
        const db = this._getDb();
        if (!db) {
            console.warn('Database not configured - cannot store agent booking URL');
            return null;
        }
        try {
            const { data, error } = await db
                ...
                .single();
            if (error) throw error;
            console.log(`Stored booking URL for agent ${agentId}: ${bookingUrl}`);
            return data;
        } catch (error) {
            console.error('Error storing agent booking URL:', error.message);
            throw error;
        }
    }
...
// Bind ALL prototype methods so destructured imports work
for (const key of Object.getOwnPropertyNames(CalcomService.prototype)) {
    if (key !== 'constructor' && typeof calcomService[key] === 'function') {
        calcomService[key] = calcomService[key].bind(calcomService);
    }
}
...
module.exports.isConfigured = calcomService.isConfigured;
module.exports.getApiKey = calcomService.getApiKey;
module.exports.calApiRequest = calcomService.calApiRequest;
module.exports.getEventTypes = calcomService.getEventTypes;
module.exports.getEventType = calcomService.getEventType;
module.exports.generateBookingUrl = calcomService.generateBookingUrl;
module.exports.getAvailableSlots = calcomService.getAvailableSlots;
module.exports.createBooking = calcomService.createBooking;
module.exports.getBooking = calcomService.getBooking;
module.exports.cancelBooking = calcomService.cancelBooking;
module.exports.rescheduleBooking = calcomService.rescheduleBooking;
module.exports.getMe = calcomService.getMe;
module.exports.getTeamMembers = calcomService.getTeamMembers;
module.exports.verifyWebhookSignature = calcomService.verifyWebhookSignature;
module.exports.handleCalWebhook = calcomService.handleCalWebhook;
module.exports.handleBookingCreated = calcomService.handleBookingCreated;
module.exports.handleBookingRescheduled = calcomService.handleBookingRescheduled;
module.exports.handleBookingCancelled = calcomService.handleBookingCancelled;
module.exports.handleMeetingEnded = calcomService.handleMeetingEnded;
module.exports.calcomWebhookHandler = calcomService.calcomWebhookHandler;
module.exports.listWebhooks = calcomService.listWebhooks;
module.exports.registerWebhook = calcomService.registerWebhook;
module.exports.deleteWebhook = calcomService.deleteWebhook;
module.exports.updateWebhook = calcomService.updateWebhook;
module.exports.getWebhook = calcomService.getWebhook;
module.exports.logWebhookDelivery = calcomService.logWebhookDelivery;
module.exports.getWebhookDeliveryLogs = calcomService.getWebhookDeliveryLogs;
module.exports.getWebhookStats = calcomService.getWebhookStats;
module.exports.testWebhook = calcomService.testWebhook;
module.exports.generateWebhookSecret = calcomService.generateWebhookSecret;
module.exports.withRetry = calcomService.withRetry;
module.exports.sleep = calcomService.sleep;
module.exports.calculateBackoffDelay = calcomService.calculateBackoffDelay;
module.exports.storeAgentBookingUrl = calcomService.storeAgentBookingUrl;
module.exports.getAgentBookingUrl = calcomService.getAgentBookingUrl;
