...
class WeeklyPerformanceService {
  ...
  async getEligibleAgents(weekStarting) {
    if (!this.isSupabaseConfigured()) {
      log.warn('[Weekly Report] Supabase not configured, returning empty agent list');
      return [];
    }
    ...
    try {
      ...
      if (error) {
        log.error('[Weekly Report] Error fetching eligible agents:', error);
        throw error;
      }

      if (!agents || agents.length === 0) {
        log.info('[Weekly Report] No active agents found');
        return [];
      }
      ...
      if (logError) {
        log.error('[Weekly Report] Error fetching email logs:', logError);
        throw logError;
      }
      ...
      log.info(`[Weekly Report] Found ${eligibleAgents.length} agents eligible for week starting ${weekStarting}`);
      return eligibleAgents;
    } catch (error) {
      log.error('[Weekly Report] Error getting eligible agents:', error);
      throw error;
    }
  }
  ...
  async getAgentWeeklyStats(agentId, weekStarting, weekEnding) {
    ...
    try {
      ...
      try {
        ...
        log.warn('[Weekly Report] Error counting leads:', null, { error: error.message });
      }
      ...
      try {
        ...
        log.warn('[Weekly Report] Error calculating response time:', null, { error: error.message });
      }
      ...
      try {
        ...
        log.warn('[Weekly Report] Error counting bookings:', null, { error: error.message });
      }
      ...
      log.error(`[Weekly Report] Error getting stats for agent ${agentId}:`, error);
      return {
        leadsResponded: 0,
        avgResponseTimeSeconds: 0,
        appointmentsBooked: 0,
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #f5f5f5;">
  ...
  async sendWeeklyEmail(agent, stats, weekRange) {
    if (!this.isResendConfigured()) {
      log.warn('[Weekly Report] Resend not configured, simulating send');
      return {
        success: true,
        messageId: `simulated-${Date.now()}`,
        provider: 'simulated'
    ...
    try {
      ...
      log.error(`[Weekly Report] Error sending email to ${agent.email}:`, error);
      throw error;
    }
  }
  ...
  async logEmailSend(agent, stats, weekRange, sendResult, status, errorMessage = null) {
    if (!this.isSupabaseConfigured()) {
      log.warn('[Weekly Report] Supabase not configured, skipping log');
      return;
    }
    ...
    try {
      ...
      if (error) {
        log.error('[Weekly Report] Error logging email send:', error);
      }
    } catch (error) {
      log.error('[Weekly Report] Error in logEmailSend:', error);
    }
  }

  async processWeeklyEmails() {
    log.info('[Weekly Report] Starting weekly performance email processing...');

    const weekRange = this.getPreviousWeekRange();
    log.info(`[Weekly Report] Processing week: ${weekRange.weekStarting} to ${weekRange.weekEnding}`);
    ...
    try {
      ...
      if (agents.length === 0) {
        log.info('[Weekly Report] No eligible agents to process');
        return results;
      }
      ...
      for (const agent of agents) {
        ...
        try {
          ...
          log.info(`[Weekly Report] ✓ Sent to ${agent.email}`);
        } catch (error) {
          const stats = {
            leadsResponded: 0,
            avgResponseTimeSeconds: 0,
          ...
          log.error(`[Weekly Report] ✗ Failed to send to ${agent.email}:`, error);
        }

        results.agents.push(agentResult);
      }

      log.info('[Weekly Report] Processing complete:', {
      ...
      log.error('[Weekly Report] Fatal error in processWeeklyEmails:', error);
      throw error;
    }
  }
  ...
  async getPreviewData(agentId) {
    ...
    if (agentId && this.isSupabaseConfigured()) {
      try {
        ...
        log.warn('[Weekly Report] Preview: could not load agent, using defaults');
      }
    }
