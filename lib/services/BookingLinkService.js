...
const CalcomClient = require('./CalcomClient');
const { logger } = require('../logger');
const log = logger.child('BookingLinkService');
...
class BookingLinkService {
    ...
    constructor(options = {}) {
        ...
        this.calcomClient = options.calcomClient || new CalcomClient();
     ...
     * @param {Object} [options]
     * @param {{ name?: string, email?: string, notes?: string }} [options.prefill]
     * @param {string} [options.utmSource]
     * @returns {Promise<Booking & { agent?: Agent, lead?: Lead, booking_url?: string }>}
     */
    async generateAgentBookingLink(agentId, eventTypeSlug, options = {}) {
        ...
        let eventType = null;
        if (this.calcomClient.isConfigured()) {
            const eventTypes = await this.calcomClient.getEventTypes({ username: calUsername });
            eventType = eventTypes.find(et => et.slug === eventTypeSlug) || null;
        }

        // 4. Generate booking URL
        let bookingUrl = this.calcomClient.generateBookingUrl(eventTypeSlug, calUsername) ||
        ...
        if (upsertError) {
            log.warn('Failed to store booking config:', null, { error: upsertError.message });
        }
    ...
    async getAgentBookingLinks(agentId) {
        ...
        if (this.calcomClient.isConfigured()) {
            try {
                freshEventTypes = await this.calcomClient.getEventTypes();
            } catch (err) {
                log.warn('Could not fetch fresh event types:', null, { error: err.message });
            }
        }
