/*
TASK SPEC (ef5c29f3-8998-4048-8971-2e60182d5a7d)
What:
- Update /lib/services/BillingService.js:
  - BillingService.getOrCreateCustomer()
  - BillingService.createCustomer()
  - Add shared statement descriptor constant for Stripe customer creation payloads.
- Add /tests/unit/billing-service-statement-descriptor.test.js to assert BillingService.createCustomer sends statement_descriptor: 'LANDYOURLEADS'.
- Update footer legal copy in:
  - /product/lead-response/dashboard/app/page.tsx
  - /product/lead-response/dashboard/app/demo/page.tsx

Verify:
- rg -n "Imagine Squared" product/lead-response/dashboard/app | rg -v "app/privacy/page.tsx|app/terms/page.tsx|footer"
- rg -n "statement_descriptor" lib/services/BillingService.js tests/unit/billing-service-statement-descriptor.test.js
- npm test
- npm run build

Boundaries:
- Do not change Stripe dashboard settings, migrations, DB schema, or API route contracts.
- Do not change privacy/terms legal body text beyond existing brand/legal alignment.
- Do not refactor unrelated services or UI components.
*/
...
const CUSTOMER_STATEMENT_DESCRIPTOR = 'LANDYOURLEADS';
...
class BillingService {
  ...
  async getOrCreateCustomer(userId) {
    ...
    const customer = await breakers.stripe.execute(() => this.stripe.customers.create({
      email: agent.email,
      name: agent.name || agent.email,
      statement_descriptor: CUSTOMER_STATEMENT_DESCRIPTOR,
      metadata: { user_id: userId, source: 'leadflow' }
    }));
  ...
  async createCustomer(agent) {
    if (!this.stripe) return { id: 'mock_customer_'+agent.id, mock: true };
    return breakers.stripe.execute(() => this.stripe.customers.create({
      email: agent.email,
      name: agent.name||agent.email,
      statement_descriptor: CUSTOMER_STATEMENT_DESCRIPTOR,
      metadata: { agent_id: agent.id, source: 'leadflow_onboarding' }
    }));
  }
