...
const { TRIAL_PERIOD_DAYS, ANNUAL_DISCOUNT_MONTHS, MONTHLY_PRICES, stripe: stripeConfig, app: appConfig } = require('../config');
...
class BillingService {
  ...
  /**
   * Create a Stripe hosted checkout session for monthly or annual billing.
   *
   * @param {object} params
   * @param {string} params.agentId        - Agent UUID
   * @param {string} params.email          - Agent email
   * @param {string} params.tier           - 'starter' | 'pro' | 'team' | 'brokerage'
   * @param {'monthly'|'annual'} [params.billingInterval] - Billing frequency (default 'monthly')
   * @param {string} [params.successUrl]   - Redirect on success
   * @param {string} [params.cancelUrl]    - Redirect on cancel
   * @returns {{ sessionId: string, url: string, tier: string, billingInterval: string, annualSavings: number }}
   */
  async createCheckoutSession(params) {
    const { agentId, email, tier, billingInterval = 'monthly', successUrl, cancelUrl } = params;

    // Map billingInterval to Stripe price interval key
    const intervalKey = billingInterval === 'annual' ? 'year' : 'month';

    // Resolve price ID from config
    const priceId = stripeConfig.prices[tier] && stripeConfig.prices[tier][intervalKey];

    if (!this.stripe) {
      // Mock path for test environments without Stripe configured
      return {
        mock: true,
        sessionId: 'mock_cs_' + tier + '_' + billingInterval + '_' + Date.now(),
        url: successUrl || (appConfig.appUrl + '/dashboard'),
        tier,
        billingInterval,
        annualSavings: billingInterval === 'annual' ? (MONTHLY_PRICES[tier] || 0) * ANNUAL_DISCOUNT_MONTHS : 0,
      };
    }

    if (!priceId) {
      const envVarName = 'STRIPE_PRICE_' + tier.toUpperCase() + '_' + (intervalKey === 'year' ? 'ANNUAL' : 'MONTHLY');
      throw new Error('No Stripe price configured for tier "' + tier + '" / interval "' + intervalKey + '". Set ' + envVarName + ' in environment.');
    }

    const baseUrl = appConfig.appUrl || 'https://leadflow-ai-five.vercel.app';
    ...
    log.info('Checkout session created', { agentId, tier, billingInterval, sessionId: session.id });
