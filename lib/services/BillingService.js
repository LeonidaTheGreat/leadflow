...
class BillingService {
  ...
  async createPromoCode({ code, discountPercent, expiryDays, tier, metadata = {} }) {
    const expiresAt = Math.floor((Date.now() + expiryDays * 86400000) / 1000);
    if (!this.stripe) {
      return { code, percent_off: discountPercent, stripe_promo_code_id: 'prmo_mock_' + Date.now(), stripe_coupon_id: 'co_mock_' + Date.now(), expiry_at: new Date(expiresAt * 1000), mock: true };
    }
    const coupon = await breakers.stripe.execute(() => this.stripe.coupons.create({ percent_off: discountPercent, duration: 'once', redeem_by: expiresAt, metadata: { tier, ...metadata } }));
    const promoCode = await breakers.stripe.execute(() => this.stripe.promotionCodes.create({ coupon: coupon.id, code, expires_at: expiresAt, max_redemptions: 1, metadata: { tier, ...metadata } }));
    return { code, percent_off: discountPercent, stripe_promo_code_id: promoCode.id, stripe_coupon_id: coupon.id, expiry_at: new Date(expiresAt * 1000) };
  }
