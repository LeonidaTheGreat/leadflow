...
class BillingService {
  ...
  async createPromoCode({ code, discountPercent, expiryDays, tier, metadata = {} }) {
    ...
    const coupon = await this.stripe.coupons.create({ percent_off: discountPercent, duration: 'once', metadata: { tier, ...metadata } });
    const promoCode = await this.stripe.promotionCodes.create({ coupon: coupon.id, code, expires_at: Math.floor(expiryAt.getTime() / 1000), metadata: { tier, ...metadata } });
    return { code, percent_off: discountPercent, stripe_promo_code_id: promoCode.id, stripe_coupon_id: coupon.id, expiry_at: expiryAt };
  }
