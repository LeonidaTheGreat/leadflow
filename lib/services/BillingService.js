...
class BillingService {
  constructor(options = {}) {
    ...
    this.db = options.db || (process.env.NEXT_PUBLIC_API_URL ? createClient(process.env.NEXT_PUBLIC_API_URL, process.env.API_SECRET_KEY || process.env.LEADFLOW_API_KEY) : null);
  }
  ...
  async initializeBilling() {
    ...
    try {
      ...
      if (process.env.STRIPE_WEBHOOK_SECRET) { status.webhooks = true; } else { status.errors.push('STRIPE_WEBHOOK_SECRET not set'); }
      for (const p of ['STRIPE_PRICE_STARTER_MONTHLY','STRIPE_PRICE_PROFESSIONAL_MONTHLY','STRIPE_PRICE_ENTERPRISE_MONTHLY']) { if (!process.env[p]) status.errors.push(p+' not set'); }
      return status;
    } catch (error) { status.errors.push(error.message); return status; }
  }
  ...
  async createManagedSubscription(params) {
    ...
    if (!this.stripe) throw new Error('BillingService: Stripe is not initialized. Set STRIPE_SECRET_KEY.');
    try {
      ...
      if (trial) sp.trial_period_days = 14;
      const ss = await this.stripe.subscriptions.create(sp);
    ...
    } catch (error) { console.error('Failed to create managed subscription:', error.message); throw error; }
  }
  ...
  async getOrCreateCustomer(userId) {
    ...
    if (agent?.stripe_customer_id) { try { return await this.stripe.customers.retrieve(agent.stripe_customer_id); } catch(e) { console.warn('Existing customer not found, creating new one'); } }
    const customer = await this.stripe.customers.create({ email: agent.email, name: agent.name || agent.email, metadata: { user_id: userId, source: 'leadflow' } });
  ...
  async changeSubscriptionPlan(params) {
    ...
    try {
      const cs = await this.stripe.subscriptions.retrieve(subscriptionId);
      ...
      const us = await this.stripe.subscriptions.update(subscriptionId, { items: [{ id: ci.id, price: np }], proration_behavior: prorationBehavior, metadata: { ...cs.metadata, plan_changed_at: Date.now().toString(), previous_tier: cs.metadata.tier, new_tier: newTier } });
    ...
    } catch (error) { console.error('Failed to change subscription plan:', error.message); throw error; }
  }
  ...
  async scheduleSubscriptionChange(params) {
    ...
    try {
      ...
      const s = await this.stripe.subscriptions.update(subscriptionId, { pending_update: { items: [{ price: np }] } });
    ...
    } catch (error) { console.error('Failed to schedule subscription change:', error.message); throw error; }
  }
  ...
  async cancelManagedSubscription(params) {
    ...
    if (!this.stripe) { result = { success: true, mock: true, status: 'cancelled', currentPeriodEnd: null }; }
      ...
      try {
        result = immediate ? await this.stripe.subscriptions.cancel(subscriptionId, { cancellation_details: { comment: feedback, feedback: reason } }) : await this.stripe.subscriptions.update(subscriptionId, { cancel_at_period_end: true, cancellation_details: { comment: feedback, feedback: reason } });
      ...
      } catch (error) { console.error('Failed to cancel subscription:', error.message); throw error; }
    }
    if (immediate && downgradeFeatures) { const s = await this.stripe?.subscriptions.retrieve(subscriptionId); if (s?.metadata?.user_id) await this._downgradeUserFeatures(s.metadata.user_id); }
  ...
  async reactivateSubscription(subscriptionId) {
    ...
    try {
      const s = await this.stripe.subscriptions.update(subscriptionId, { cancel_at_period_end: false });
    ...
    } catch (error) { console.error('Failed to reactivate subscription:', error.message); throw error; }
  }
  ...
  async getSubscriptionDetails(subscriptionId) {
    ...
    if (this.stripe && sub.stripe_subscription_id) { try { const ss = await this.stripe.subscriptions.retrieve(sub.stripe_subscription_id); stripeStatus = { status: ss.status, currentPeriodEnd: ss.current_period_end, cancelAtPeriodEnd: ss.cancel_at_period_end }; } catch(e) { console.warn('Could not fetch Stripe status:', e.message); } }
  ...
  async getBillingCycleInfo(subscriptionId) {
    ...
    try {
      ...
      let ui = null; try { ui = await this.stripe.invoices.retrieveUpcoming({ subscription: subscriptionId }); } catch(e) {}
    ...
    } catch (error) { console.error('Failed to get billing cycle info:', error.message); throw error; }
  }
  ...
  async calculateProration(params) {
    ...
    try {
      ...
      const ui = await this.stripe.invoices.retrieveUpcoming({ subscription: subscriptionId, subscription_items: [{ id: sub.items.data[0].id, price: newPriceId }], subscription_proration_date: prorationDate || Math.floor(Date.now()/1000) });
    ...
    } catch (error) { console.error('Failed to calculate proration:', error.message); throw error; }
  }
  ...
  async previewBillingCycleChange(params) {
    ...
    try {
      ...
      const np = TIER_PRICES[newTier]?.[ti]; if (!np) throw new Error('Invalid tier '+newTier);
      const ui = await this.stripe.invoices.retrieveUpcoming({ subscription: subscriptionId, subscription_items: [{ id: ci.id, price: np }] });
      const newPrice = await this.stripe.prices.retrieve(np, { expand: ['product'] });
    ...
    } catch (error) { console.error('Failed to preview billing cycle:', error.message); throw error; }
  }
  ...
  async updateBillingCycle(params) {
    ...
    catch (error) { console.error('Failed to update billing cycle:', error.message); throw error; }
  }
  ...
  async getRenewalHistory(subscriptionId) {
    ...
    } catch (error) { console.error('Failed to get renewal history:', error.message); throw error; }
  }
  ...
  async getUpcomingRenewals(options = {}) {
    ...
    } catch (error) { console.error('Failed to get upcoming renewals:', error.message); throw error; }
  }
  ...
  async syncBillingCycles(subscriptionId = null) {
    ...
    try {
      const subs = subscriptionId ? [await this.stripe.subscriptions.retrieve(subscriptionId)] : (await this.stripe.subscriptions.list({ status: 'all', limit: 100 })).data;
      let synced = 0;
    ...
    } catch (error) { console.error('Failed to sync billing cycles:', error.message); throw error; }
  }
  ...
  async configurePortal() {
    ...
    try {
      const cfgs = await this.stripe.billingPortal.configurations.list({ limit: 1 });
      ...
      const cfg = cfgs.data.length > 0 ? await this.stripe.billingPortal.configurations.update(cfgs.data[0].id, cd) : await this.stripe.billingPortal.configurations.create(cd);
      return { success: true, configurationId: cfg.id, config: PORTAL_CONFIG };
    } catch (error) { console.error('Failed to configure Stripe Customer Portal:', error.message); throw error; }
  }
  ...
  async createPortalSession(customerId, options = {}) {
    ...
    if (!this.stripe) return { mock: true, url: options.returnUrl || 'https://leadflow.ai/dashboard', customerId };
    try {
      const sc = { customer: customerId, return_url: options.returnUrl || process.env.STRIPE_PORTAL_RETURN_URL || 'https://leadflow.ai/dashboard' };
      if (options.locale) sc.locale = options.locale;
      if (options.configurationId) sc.configuration = options.configurationId;
      const session = await this.stripe.billingPortal.sessions.create(sc);
      return { success: true, url: session.url, sessionId: session.id, customerId: session.customer };
    } catch (error) { console.error('Failed to create portal session:', error.message); throw error; }
  }

  getPortalConfig() { return { ...PORTAL_CONFIG, returnUrl: process.env.STRIPE_PORTAL_RETURN_URL || 'https://leadflow.ai/dashboard' }; }
  ...
  async getCustomerSubscriptions(customerId) {
    ...
    try { const subs = await this.stripe.subscriptions.list({ customer: customerId, status: 'all', expand: ['data.items.price.product'] }); return { success: true, subscriptions: subs.data.map(s => ({ id: s.id, status: s.status, plan: s.items.data[0]?.price?.product?.name||'Unknown Plan', amount: s.items.data[0]?.price?.unit_amount/100, currency: s.currency, interval: s.items.data[0]?.price?.recurring?.interval, currentPeriodStart: s.current_period_start, currentPeriodEnd: s.current_period_end, cancelAtPeriodEnd: s.cancel_at_period_end, canceledAt: s.canceled_at, endedAt: s.ended_at })) }; }
    catch (error) { console.error('Failed to get customer subscriptions:', error.message); throw error; }
  }

  async getCustomerInvoices(customerId, options = {}) {
    if (!this.stripe || customerId.startsWith('mock_')) return { mock: true, invoices: [{ id: 'mock_inv_123', number: 'INV-001', amountDue: 997, amountPaid: 997, currency: 'usd', status: 'paid', created: Date.now()/1000-86400*15, periodStart: Date.now()/1000-86400*30, periodEnd: Date.now()/1000-86400, pdfUrl: 'https://leadflow.ai/mock-invoice.pdf' }] };
    try { const invs = await this.stripe.invoices.list({ customer: customerId, limit: options.limit||24, starting_after: options.startingAfter }); return { success: true, invoices: invs.data.map(i => ({ id: i.id, number: i.number, amountDue: i.amount_due/100, amountPaid: i.amount_paid/100, currency: i.currency, status: i.status, created: i.created, periodStart: i.period_start, periodEnd: i.period_end, pdfUrl: i.invoice_pdf, hostedInvoiceUrl: i.hosted_invoice_url })), hasMore: invs.has_more }; }
    catch (error) { console.error('Failed to get customer invoices:', error.message); throw error; }
  }
  ...
  async getCustomerPaymentMethods(customerId) {
    ...
    try { const c = await this.stripe.customers.retrieve(customerId); const dpm = c.invoice_settings?.default_payment_method; const pms = await this.stripe.paymentMethods.list({ customer: customerId, type: 'card' }); return { success: true, defaultPaymentMethod: dpm, paymentMethods: pms.data.map(pm => ({ id: pm.id, type: pm.type, brand: pm.card?.brand, last4: pm.card?.last4, expMonth: pm.card?.exp_month, expYear: pm.card?.exp_year, isDefault: pm.id === dpm })) }; }
    catch (error) { console.error('Failed to get customer payment methods:', error.message); throw error; }
  }
  ...
  async processWebhookEvent(event) {
    ...
    } catch (error) { console.error('Error processing webhook '+event.type+':', error.message); await this._logWebhookError(event, error); throw error; }
  }

  verifyWebhookSignature(payload, signature) {
    if (!this.stripe || !process.env.STRIPE_WEBHOOK_SECRET) throw new Error('Webhook verification not configured');
  ...
  async handleSubscriptionUpdated(subscription) {
    ...
    if (subscription.status === 'past_due') { const u = subscription.metadata?.user_id; if (u) console.log('Handling past due subscription: '+subscription.id); }
  ...
  async handleSubscriptionDeleted(subscription) {
    ...
    if (uid) { await this._updateAgentStatus(uid, { subscriptionStatus: 'cancelled', subscriptionTier: null, currentPeriodEnd: null }); console.log('Handling account downgrade for user: '+uid); }
  ...
  async handleTrialWillEnd(subscription) { const uid = subscription.metadata?.user_id; if (uid) console.log('Sending trial ending notification to user: '+uid); return { acknowledged: true, notificationSent: !!uid }; }
  ...
  async handlePaymentFailed(invoice) {
    ...
    if (invoice.subscription && this.db) { const { data: sub } = await this.db.from('subscriptions').select('user_id').eq('stripe_subscription_id', invoice.subscription).single(); if (sub?.user_id) console.log('Sending payment failed notification to user: '+sub.user_id); }
    return { acknowledged: true, recorded: true };
  }

  async handleInvoicePaid() { return { acknowledged: true }; }
  async handlePaymentActionRequired(invoice) { if (invoice.subscription && this.db) { const { data: sub } = await this.db.from('subscriptions').select('user_id').eq('stripe_subscription_id', invoice.subscription).single(); if (sub?.user_id) console.log('Sending payment action required notification'); } return { acknowledged: true }; }
  async handleInvoiceCreated() { return { acknowledged: true }; }
  async handleInvoiceFinalized() { return { acknowledged: true }; }
  async handleInvoiceUpcoming(invoice) { if (invoice.subscription && this.db) { const { data: sub } = await this.db.from('subscriptions').select('user_id').eq('stripe_subscription_id', invoice.subscription).single(); if (sub?.user_id) console.log('Sending upcoming invoice notification'); } return { acknowledged: true }; }
  ...
  async handleCheckoutCompleted(session) {
    ...
    if (this.db && this.stripe && userId && subscriptionId) {
      try {
        const s = await this.stripe.subscriptions.retrieve(subscriptionId);
        const li = s.items.data[0], pi = li?.price?.id||'unknown', iv = li?.price?.recurring?.interval||'month', tier = s.metadata?.tier||'professional';
        ...
        if (error) console.error('Failed to upsert subscription from checkout:', error.message);
        await this.db.from('real_estate_agents').update({ stripe_customer_id: scid, subscription_status: s.status, plan_tier: tier, updated_at: new Date() }).eq('id', userId);
      } catch (err) { console.error('Error persisting subscription from checkout:', err.message); }
    }
  ...
  // ==================== SIMPLE STRIPE ====================
  async createCustomer(agent) { if (!this.stripe) return { id: 'mock_customer_'+agent.id, mock: true }; return this.stripe.customers.create({ email: agent.email, name: agent.name||agent.email, metadata: { agent_id: agent.id, source: 'leadflow_onboarding' } }); }
  async createSubscription(customerId, priceId) { if (!this.stripe) return { id: 'mock_sub_'+Date.now(), mock: true, status: 'active' }; const dp = process.env.STRIPE_PRICE_BASIC||priceId; if (!dp) throw new Error('No price ID provided and STRIPE_PRICE_BASIC not set'); return this.stripe.subscriptions.create({ customer: customerId, items: [{ price: dp }], payment_behavior: 'default_incomplete', payment_settings: { save_default_payment_method: 'on_subscription' }, expand: ['latest_invoice.payment_intent'], metadata: { source: 'leadflow_agent_signup' } }); }
  async attachPaymentMethod(customerId, paymentMethodId) { if (!this.stripe) return { success: true, mock: true }; await this.stripe.paymentMethods.attach(paymentMethodId, { customer: customerId }); await this.stripe.customers.update(customerId, { invoice_settings: { default_payment_method: paymentMethodId } }); return { success: true }; }
  async createSetupIntent(customerId) { if (!this.stripe) return { client_secret: 'mock_secret_'+Date.now(), mock: true }; const si = await this.stripe.setupIntents.create({ customer: customerId, usage: 'off_session', automatic_payment_methods: { enabled: true } }); return { client_secret: si.client_secret }; }
  async getSubscriptionStatus(subscriptionId) { if (!this.stripe || subscriptionId.startsWith('mock_')) return { status: 'active', mock: true }; const s = await this.stripe.subscriptions.retrieve(subscriptionId); return { status: s.status, current_period_end: s.current_period_end, cancel_at_period_end: s.cancel_at_period_end }; }
  async cancelSubscription(subscriptionId, immediate = false) { if (!this.stripe || subscriptionId.startsWith('mock_')) return { status: 'cancelled', mock: true }; return immediate ? this.stripe.subscriptions.cancel(subscriptionId) : this.stripe.subscriptions.update(subscriptionId, { cancel_at_period_end: true }); }
  ...
  async _downgradeUserFeatures(userId) { console.log('Downgrading features for user: '+userId); }
  ...
  async _logWebhookEvent(event) { if (!this.db) return; try { await this.db.from('subscription_events').insert({ stripe_event_id: event.id, event_type: event.type, stripe_event_data: event, processed_at: new Date() }); } catch(e) { console.error('Failed to log webhook event:', e.message); } }
  async _markEventProcessed(eventId) { if (!this.db) return; try { await this.db.from('subscription_events').update({ processed_at: new Date() }).eq('stripe_event_id', eventId); } catch(e) { console.error('Failed to mark event processed:', e.message); } }
  async _logWebhookError(event, error) { if (!this.db) return; try { await this.db.from('subscription_events').update({ processing_error: error.message, error_at: new Date() }).eq('stripe_event_id', event.id); } catch(e) { console.error('Failed to log webhook error:', e.message); } }
