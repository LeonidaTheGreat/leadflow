...
class BillingService {
  ...
  async getOrCreateCustomer(userId) {
    ...
    if (agent?.stripe_customer_id) { try { return await this.stripe.customers.retrieve(agent.stripe_customer_id); } catch(e) { log.warn('Existing customer not found, creating new one'); } }
  ...
  async getSubscriptionDetails(subscriptionId) {
    ...
    if (this.stripe && sub.stripe_subscription_id) { try { const ss = await this.stripe.subscriptions.retrieve(sub.stripe_subscription_id); stripeStatus = { status: ss.status, currentPeriodEnd: ss.current_period_end, cancelAtPeriodEnd: ss.cancel_at_period_end }; } catch(e) { log.warn('Could not fetch Stripe status:', null, { error: e.message }); } }
  ...
  async processWebhookEvent(event) {
    ...
    log.info('Processing webhook: '+event.type+' ('+event.id+')');
    try {
    ...
    } catch (error) { log.error('Error processing webhook '+event.type+':', error); await this._logWebhookError(event, error); throw error; }
  }
  ...
  async handleSubscriptionUpdated(subscription) {
    ...
    if (subscription.status === 'past_due') { const u = subscription.metadata?.user_id; if (u) log.info('Handling past due subscription: '+subscription.id); }
  ...
  async handleSubscriptionDeleted(subscription) {
    ...
    if (uid) { await this._updateAgentStatus(uid, { subscriptionStatus: 'cancelled', subscriptionTier: null, currentPeriodEnd: null }); log.info('Handling account downgrade for user: '+uid); }
  ...
  async handleTrialWillEnd(subscription) { const uid = subscription.metadata?.user_id; if (uid) log.info('Sending trial ending notification to user: '+uid); return { acknowledged: true, notificationSent: !!uid }; }
  ...
  async handlePaymentFailed(invoice) {
    ...
    if (invoice.subscription && this.db) { const { data: sub } = await this.db.from('subscriptions').select('user_id').eq('stripe_subscription_id', invoice.subscription).single(); if (sub?.user_id) log.info('Sending payment failed notification to user: '+sub.user_id); }
    return { acknowledged: true, recorded: true };
  }

  async handleInvoicePaid() { return { acknowledged: true }; }
  async handlePaymentActionRequired(invoice) { if (invoice.subscription && this.db) { const { data: sub } = await this.db.from('subscriptions').select('user_id').eq('stripe_subscription_id', invoice.subscription).single(); if (sub?.user_id) log.info('Sending payment action required notification'); } return { acknowledged: true }; }
  async handleInvoiceCreated() { return { acknowledged: true }; }
  async handleInvoiceFinalized() { return { acknowledged: true }; }
  async handleInvoiceUpcoming(invoice) { if (invoice.subscription && this.db) { const { data: sub } = await this.db.from('subscriptions').select('user_id').eq('stripe_subscription_id', invoice.subscription).single(); if (sub?.user_id) log.info('Sending upcoming invoice notification'); } return { acknowledged: true }; }
