...
class BillingService {
  ...
  async initializeBilling() {
    ...
    try {
      ...
      for (const [tier, key] of [['starter','STRIPE_PRICE_STARTER_MONTHLY'],['professional','STRIPE_PRICE_PRO_MONTHLY'],['enterprise','STRIPE_PRICE_TEAM_MONTHLY']]) { if (!TIER_PRICES[tier]?.month) status.errors.push(key+' not set'); }
      return status;
  ...
  async _logWebhookEvent(event) { if (!this.db) return; try { await this.db.from('subscription_events').insert({ stripe_event_id: event.id, event_type: event.type, stripe_event_data: event, processed_at: new Date() }); } catch(e) { log.error('Failed to log webhook event', e); } try { await this.db.from('stripe_events').insert({ type: event.type, payload: event, received_at: new Date() }); } catch(e) { log.error('Failed to log to stripe_events', e); } }
...
// Bound export so destructuring { processWebhookEvent } retains correct `this`
