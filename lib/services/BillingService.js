...
const { TRIAL_PERIOD_DAYS } = require('../config');
...
class BillingService {
  ...
  // ==================== SIMPLE STRIPE ====================
  async createCustomer(agent) { if (!this.stripe) return { id: 'mock_customer_'+agent.id, mock: true }; return breakers.stripe.execute(() => this.stripe.customers.create({ email: agent.email, name: agent.name||agent.email, metadata: { agent_id: agent.id, source: 'leadflow_onboarding' } })); }
