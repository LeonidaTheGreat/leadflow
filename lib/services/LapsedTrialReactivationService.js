...
const COUNT_SENT_SQL = `
  SELECT COUNT(*)::integer AS count
  FROM real_estate_agents
  WHERE COALESCE(trial_email_expired_sent, false) = true
`;
...
class LapsedTrialReactivationService {
  ...
  async getStats() {
    ...
    const [{ rows: eligibleRows }, { rows: sentRows }] = await Promise.all([
      this.pool.query(COUNT_ELIGIBLE_SQL),
      this.pool.query(COUNT_SENT_SQL),
    ]);

    return {
      eligible: (eligibleRows[0] && eligibleRows[0].count) || 0,
      sent: (sentRows[0] && sentRows[0].count) || 0,
    };
  }
