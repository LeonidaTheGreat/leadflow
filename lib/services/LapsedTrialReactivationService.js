...
const STATS_SQL = `
  SELECT
    COUNT(*) FILTER (WHERE trial_email_expired_sent = true)::integer AS sent,
    COUNT(*) FILTER (
      WHERE trial_email_expired_sent = true
        AND utm_campaign = 'lapsed_trial_reactivation'
    )::integer AS opened_via_utm,
    COUNT(*) FILTER (
      WHERE trial_email_expired_sent = true
        AND subscription_status = 'active'
    )::integer AS reactivated
  FROM real_estate_agents
  WHERE trial_ends_at IS NOT NULL
    AND trial_ends_at < NOW()
    AND COALESCE(aha_completed, false) = false
...
class LapsedTrialReactivationService {
  constructor(options = {}) {
    ...
    this.emailService = options.emailService || new EmailService({
      fromEmail: options.fromEmail || process.env.FROM_EMAIL || 'stojan@landyourleads.com',
    });
  ...
  async getStats() {
    ...
    const [countResult, statsResult] = await Promise.all([
      this.pool.query(COUNT_ELIGIBLE_SQL),
      this.pool.query(STATS_SQL),
    ]);

    const eligible = (countResult.rows[0] && countResult.rows[0].count) || 0;
    const stats = statsResult.rows[0] || { sent: 0, opened_via_utm: 0, reactivated: 0 };

    return {
      eligible,
      sent: stats.sent,
      opened_via_utm: stats.opened_via_utm,
