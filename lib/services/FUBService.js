...
const findLeadByPhone = _defaultSequenceService.findLeadByPhone.bind(_defaultSequenceService);
...
class FUBService {
  constructor(options = {}) {
    ...
    this.findLeadByPhone = options.findLeadByPhone || findLeadByPhone;
  ...
  async handleLeadCreated(leadData) {
    ...
    try {
      if (!leadData.phoneNumber) {
        this.logger.warn(`⚠️  Lead ${leadData.id} missing phone number, skipping SMS and enrollment`);
        return;
      }

      // fullLead is resolved inside the SMS try-block; used for enrollment fallback below
      let fullLead = null;

      try {
        fullLead = await this.fetchLeadFromFub(leadData.id);
        ...
        this.logger.error('❌ Error sending initial SMS for lead.created:', smsError.message);
        // Fall through to sequence enrollment below
      }

      // Enroll in no_response sequence regardless of SMS outcome.
      // Try fub_id lookup first, then phone fallback for leads without fub_id in DB.
      const fubId = fullLead ? String(fullLead.id) : String(leadData.id || '');
      const internalLeadId =
        (fubId ? await this.findLeadByFubId(fubId) : null) ||
        await this.findLeadByPhone(leadData.phoneNumber);
      ...
      if (internalLeadId) {
        await this.createLeadSequence({
          ...
          metadata: { fub_id: fubId, triggered_by: 'lead.created' }
        });
      } else {
        this.logger.warn(`⚠️  Could not create no_response sequence: lead not found in DB for fub_id=${leadData.id}`);
      }
    } catch (error) {
