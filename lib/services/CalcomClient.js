...
class CalcomClient {
    ...
    getApiKey() {
        return this.apiKey || config.calcom.apiKey || null;
    }
    ...
    async calApiRequest(endpoint, options = {}) {
        ...
        try {
            const response = await breakers.calcom.execute(() => this.httpClient({
                url,
            ...
            const enhancedError = new Error(
                `Cal.com API error: ${error.response?.status || 'Unknown'}`
            );
    ...
    generateBookingUrl(eventSlug, username) {
        const calUsername = username || config.calcom.username || this.defaultUsername;
...
function getMockEventTypes(defaultUsername = config.calcom.username) {
