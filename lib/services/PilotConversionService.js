...
class PilotConversionService {
  ...
  async getEligibleAgents(milestone) {
    if (!this.isDbConfigured()) { console.warn('DB not configured'); return []; }
    ...
    if (error) { console.error('Error fetching eligible agents:', error); throw error; }
    ...
    if (logError) { console.error('Error fetching email logs:', logError); throw logError; }
  ...
  async hasAgentUpgraded(agentId) {
    ...
    try {
      ...
      if (error) { console.error('Error checking agent plan tier:', error); return false; }
      return data && data.plan_tier !== 'pilot';
    } catch (error) { console.error('Error in hasAgentUpgraded:', error); return false; }
  }
  ...
  async logEmailSend(params) {
    if (!this.isDbConfigured()) { console.log('[MOCK LOG] Email send logged:', params); return; }
    try {
    ...
    } catch (error) { console.error('Error in logEmailSend:', error); }
  }
