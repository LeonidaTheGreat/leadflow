...
class PilotConversionService {
  ...
  async getEligibleAgents(milestone) {
    ...
    if (error) { log.error('Error fetching eligible agents:', error); throw error; }
    ...
    if (logError) { log.error('Error fetching email logs:', logError); throw logError; }
    ...
    log.info(`[Pilot Conversion] Found ${eligibleAgents.length} agents eligible for ${milestone}`);
  ...
  async getAgentStats(agentId) {
    ...
    try {
      ...
      log.error(`Error getting stats for agent ${agentId}:`, error);
  ...
  async hasAgentUpgraded(agentId) {
    ...
    try {
      ...
      if (error) { log.error('Error checking agent plan tier:', error); return false; }
      return data && data.plan_tier !== 'pilot';
    } catch (error) { log.error('Error in hasAgentUpgraded:', error); return false; }
  }
  ...
  async logEmailSend(params) {
    if (!this.isDbConfigured()) { log.info('[MOCK LOG] Email send logged:', { params }); return; }
    try {
  ...
  async sendConversionEmail(agent, milestone) {
    ...
    try {
      ...
      log.info(`[Pilot Conversion] Sent ${milestone} email to ${agent.email}`);
      return { success: true, messageId: sendResult.id };
    } catch (error) {
      log.error(`[Pilot Conversion] Failed to send ${milestone} to ${agent.email}:`, error);
  ...
  async processMilestone(milestone) {
    log.info(`[Pilot Conversion] Processing ${milestone}...`);
    ...
    try {
      ...
      log.error(`[Pilot Conversion] Error processing ${milestone}:`, error);
  ...
  async runConversionSequence() {
    log.info('[Pilot Conversion] Starting conversion sequence check...');
    ...
    log.info('[Pilot Conversion] Sequence check complete');
    return results;
  }
}
