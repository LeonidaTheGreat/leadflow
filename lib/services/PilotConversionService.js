...
class PilotConversionService {
  constructor(options = {}) {
    ...
    const apiKey = options.apiKey || process.env.API_SECRET_KEY || process.env.LEADFLOW_API_KEY;

    this.db = (apiUrl && apiKey) ? createClient(apiUrl, apiKey) : null;
    ...
    this.fromEmail = options.fromEmail ||
      (process.env.FROM_EMAIL || 'stojan@leadflow.ai').trim();
    this.fromName = options.fromName || 'LeadFlow';
    this.stripePricePro = options.stripePricePro || process.env.STRIPE_PRICE_PRO;
  ...
  async getEligibleAgents(milestone) {
    if (!this.isConfigured()) {
      console.warn('DB not configured, returning empty agent list');
      return [];
    }
    ...
    try {
      ...
      if (leadsData && leadsData.length > 0) {
        const responseTimes = leadsData
          ...
          .map(lead => (new Date(lead.responded_at) - new Date(lead.created_at)) / 1000);
      ...
      return {
        leadsResponded,
        avgResponseTime,
        appointmentsBooked: bookingsData?.length || 0
      };
...
`
  ...
  /**
   * Run the full conversion sequence check across all milestones
   */
...
// Singleton instance for module-level function exports
const _instance = new PilotConversionService();

module.exports = {
  PilotConversionService,
  MILESTONES,
  runConversionSequence: _instance.runConversionSequence.bind(_instance),
  processMilestone: _instance.processMilestone.bind(_instance),
  sendConversionEmail: _instance.sendConversionEmail.bind(_instance),
  getEligibleAgents: _instance.getEligibleAgents.bind(_instance),
  getAgentStats: _instance.getAgentStats.bind(_instance),
  hasAgentUpgraded: _instance.hasAgentUpgraded.bind(_instance),
  isSupabaseConfigured: () => _instance.isConfigured(),
  isResendConfigured: () => _instance.isResendConfigured()
};
