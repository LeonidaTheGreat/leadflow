...
class TwilioService {
  ...
  async sendSms(toNumber, messageContent, options = {}) {
    ...
    console.info(`📤 Sending SMS via Twilio to ***${String(toNumber).slice(-4)}`);
    ...
    try {
      ...
      console.info(`   Provisioning mode: ${mode}`);
      ...
      console.info(`   SID: ${message.sid}`);
      console.info(`   Status: ${message.status}`);
  ...
  async _logSmsToDatabase(result, metadata = {}) {
    ...
    try {
      ...
      console.info(`💾 SMS logged to database (ID: ${data.id})`);
  ...
  async updateSmsStatus(statusData) {
    ...
    console.info(`🔄 Updating SMS status: ${MessageSid} → ${MessageStatus}`);
    ...
    try {
      ...
      console.info(`✅ SMS status updated: ${MessageSid} → ${MessageStatus}`);
      return data;
