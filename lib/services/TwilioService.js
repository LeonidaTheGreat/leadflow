...
class TwilioService {
  ...
  constructor(options = {}) {
    ...
    const twilioPhoneNumberUs = cleanEnvValue(config.twilio.phoneNumberUs);
    const twilioPhoneNumberCa = cleanEnvValue(config.twilio.phoneNumberCa);
    const twilioPhoneNumberLegacy = cleanEnvValue(config.twilio.phoneNumberLegacy);
  ...
  async resolveTwilioContext(agentId, toNumber, market) {
    if (agentId) {
      try {
        ...
        log.warn('Could not load agent integrations, using platform credentials', { message: err.message });
      }
    }
   ...
   * @param {{ leadId?: Lead['id'], agentId?: Agent['id'], trigger?: string, market?: string }} options - Additional options
   * @returns {Promise<Message & { sid?: string, status?: string, errorCode?: number, errorMessage?: string, mode?: 'platform'|'customer' }>} SMS result with sid, status, and metadata
   */
  async sendSms(toNumber, messageContent, options = {}) {
    const startTime = Date.now();
    const { leadId, agentId, trigger, market, requestId } = options;
    const reqId = requestId || getRequestId();

    log.info('Sending SMS via Twilio', { toSuffix: String(toNumber).slice(-4), requestId: reqId });
    ...
    try {
      ...
      log.info('Provisioning mode resolved', { mode });

      const truncatedMessage = this.truncateMessage(messageContent);
      if (truncatedMessage !== messageContent) {
        log.warn('Message truncated', { originalLength: messageContent.length, truncatedLength: truncatedMessage.length });
      }
      ...
      const message = await breakers.twilio.execute(() => client.messages.create(messageParams));

      const duration = Date.now() - startTime;

      log.info('SMS sent successfully', { sid: message.sid, status: message.status, durationMs: duration });
      ...
      this._logSmsToDatabase(result, { leadId, agentId, trigger, market })
        .catch(err => log.error('Failed to log SMS to database', err));

      return result;

    } catch (error) {
      const duration = Date.now() - startTime;
      log.error('SMS send failed', error, { durationMs: duration });
      ...
      this._logSmsToDatabase(failedResult, { leadId, agentId, trigger, market, error: errorInfo })
        .catch(err => log.error('Failed to log SMS error to database', err));
  ...
  validateSmsInput(toNumber, messageContent) {
    ...
    if (!hasStopLanguage) {
      log.warn('SMS missing STOP language - may violate TCPA compliance');
    }
  }
  ...
  selectFromNumber(market, toNumber) {
    if (market) {
      ...
      if (marketLower.includes('ca') || marketLower.includes('canada')) {
        ...
        log.warn('CA phone number not configured, using US number');
      }
    }
  ...
  static classifyTwilioError(error) {
    ...
    if (TwilioService.isA2pBlockedError(error)) {
      return {
        ...
        message: `SMS blocked by carrier — A2P 10DLC registration required (error ${code})`,
        retryable: false,
      };
    }
    ...
    if (code === TwilioErrorCodes.INVALID_TO_NUMBER ||
      ...
      return {
        ...
        message: 'Invalid or non-SMS-capable phone number',
        retryable: false,
      };
    }
  ...
  async _logSmsToDatabase(result, metadata = {}) {
    ...
    try {
      ...
      log.info('SMS logged to database', { id: data.id });
      ...
      }).catch(e => log.error('Failed to log event', e));

      return data;

    } catch (err) {
      log.error('Failed to log SMS to database', err);
      return null;
    }
  }
  ...
  async updateSmsStatus(statusData) {
    ...
    log.info('Updating SMS status', { sid: MessageSid, status: MessageStatus });
    ...
    try {
      ...
      if (error) {
        log.error('Failed to update SMS status', error);
        return null;
      }

      log.info('SMS status updated', { sid: MessageSid, status: MessageStatus });
      return data;

    } catch (err) {
      log.error('Error updating SMS status', err);
      return null;
    }
  }
  ...
  async getSmsStatus(sid) {
    ...
    try {
      const message = await breakers.twilio.execute(() => client.messages(sid).fetch());
      ...
      log.error('Failed to fetch SMS status', error, { sid });
      throw error;
    }
  }
  ...
  async getSmsHistoryForLead(leadId, options = {}) {
    ...
    try {
      ...
      if (error) {
        log.error('Failed to fetch SMS history', error);
        throw error;
      }
      ...
      log.error('Error fetching SMS history', err);
      throw err;
    }
  }
  ...
  async getSmsAnalytics(agentId, options = {}) {
    ...
    try {
      ...
      if (error) {
        log.error('Failed to fetch SMS analytics', error);
        throw error;
      }
      ...
      log.error('Error fetching SMS analytics', err);
      throw err;
    }
  }
}
