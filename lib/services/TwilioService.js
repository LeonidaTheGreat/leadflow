...
class TwilioService {
  ...
  async resolveTwilioContext(agentId, toNumber, market) {
    if (agentId) {
      try {
        ...
        log.warn('[TwilioService] Could not load agent integrations, using platform credentials:', null, { error: err.message });
      }
    }
  ...
  async sendSms(toNumber, messageContent, options = {}) {
    ...
    log.info(`📤 Sending SMS via Twilio to ***${String(toNumber).slice(-4)}`);
    ...
    try {
      ...
      log.info(`   Provisioning mode: ${mode}`);

      const truncatedMessage = this.truncateMessage(messageContent);
      if (truncatedMessage !== messageContent) {
        log.warn(`⚠️  Message truncated from ${messageContent.length} to ${truncatedMessage.length} chars`);
      }
      ...
      const duration = Date.now() - startTime;
      log.error(`❌ SMS send failed after ${duration}ms:`, error);
  ...
  validateSmsInput(toNumber, messageContent) {
    ...
    if (!hasStopLanguage) {
      log.warn(`⚠️  SMS missing STOP language - may violate TCPA compliance`);
    }
  }
  ...
  selectFromNumber(market, toNumber) {
    if (market) {
      ...
      if (marketLower.includes('ca') || marketLower.includes('canada')) {
        ...
        log.warn(`⚠️  CA phone number not configured, using US number`);
      }
    }
  ...
  async updateSmsStatus(statusData) {
    ...
    log.info(`🔄 Updating SMS status: ${MessageSid} → ${MessageStatus}`);
    ...
    try {
      ...
      log.info(`✅ SMS status updated: ${MessageSid} → ${MessageStatus}`);
      return data;
  ...
  async getSmsStatus(sid) {
    ...
    try {
      ...
      log.error(`❌ Failed to fetch SMS status for ${sid}:`, error);
      throw error;
    }
  }
