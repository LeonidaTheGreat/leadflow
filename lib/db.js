 ...
 *   - createClient(url, key) — PostgREST HTTP client (drop-in for @supabase/supabase-js)
 *   - getPool()              — shared pg Pool for raw SQL (lazy-initialized)
 *
 * Replaces: lib/postgrest-client.js, lib/db-client.js, lib/pg-pool.js
 */
...
class QueryBuilder {
  ...
  not(key, op, val) {
    if (op !== undefined && val === undefined) {
      ...
      if (op === 'is' && val === null) {
        this._filters.push({ key, op: 'not.is.null', val: null });
      } else {
...
/**
 * Create a PostgREST HTTP client. Drop-in replacement for @supabase/supabase-js createClient().
 *
 * @param {string} [url] - PostgREST base URL. Defaults to NEXT_PUBLIC_API_URL env var.
 * @param {string} [key] - API key. Defaults to API_SECRET_KEY or LEADFLOW_API_KEY env var.
 * @returns {{ from, rpc, auth }}
 */
function createClient(url, key) {
  const finalUrl = (url || process.env.NEXT_PUBLIC_API_URL || '').trim();
  const finalKey = (key || process.env.API_SECRET_KEY || process.env.LEADFLOW_API_KEY || '').trim();
...
module.exports = { createClient, getPool };
