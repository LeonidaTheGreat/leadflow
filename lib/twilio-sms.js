...
// Initialize Twilio client (lazy initialization with fallback)
let twilioClient;
let twilioClientInitialized = false;

function getTwilioClient() {
  if (!twilioClientInitialized) {
...
// Initialize Supabase client for logging (lazy initialization with fallback)
let supabase;
...
function getSupabaseClient() {
  if (!supabaseInitialized) {
    ...
    try {
      ...
      console.warn('[Twilio SMS] Failed to initialize Supabase client:', err.message);
    }
  }
  return supabase;
}
...
async function sendSmsViatwilio(toNumber, messageContent, options = {}) {
  ...
  if (!client) {
    const error = new Error('Twilio client not initialized. Missing TWILIO_ACCOUNT_SID or TWILIO_AUTH_TOKEN');
...
async function logSmsToDatabase(result, metadata = {}) {
  ...
  // Skip logging if Supabase is not initialized
  const dbClient = getSupabaseClient();
  if (!dbClient) {
    console.warn('[Twilio SMS] Supabase not initialized, skipping database logging');
    return null;
  }
  ...
  try {
    ...
    await dbClient.from('events').insert({
      lead_id: leadId || null,
