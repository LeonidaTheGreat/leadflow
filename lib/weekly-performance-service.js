...
const supabase = (apiUrl && apiKey) ? createClient(apiUrl + '/rest/v1', apiKey) : null;
...
async function getAgentWeeklyStats(agentId, weekStarting, weekEnding) {
  ...
  try {
    ...
    // Note: sms_messages has no agent_id column, so join through leads
    ...
    try {
      ...
      if (leadIds.length > 0) {
        // Now count outbound sms_messages for these leads
...
async function getPreviewData(agentId) {
  ...
  if (agentId && isSupabaseConfigured()) {
    try {
      const { data } = await supabase.from('real_estate_agents').select('*').eq('id', agentId).single();
      if (data) agent = data;
    } catch (e) {
