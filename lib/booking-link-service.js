...
async function generateAgentBookingLink(agentId, eventTypeSlug, options = {}) {
    ...
    try {
        ...
        if (calcomService.isConfigured()) {
            const eventTypes = await calcomService.getEventTypes({ username: calUsername });
            eventType = eventTypes.find(et => et.slug === eventTypeSlug);
        }

        // 4. Generate booking URL
        let bookingUrl = calcomService.generateBookingUrl(eventTypeSlug, calUsername);
...
async function getAgentBookingLinks(agentId) {
    ...
    try {
        ...
        if (calcomService.isConfigured()) {
            try {
                freshEventTypes = await calcomService.getEventTypes();
            } catch (err) {
