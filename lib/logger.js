 ...
 * taskSpec
 * What:
 * - Update /tmp/leadflow-024a786b/lib/logger.js requestLogger()
 *   to generate fallback request IDs with crypto-backed entropy.
 * - Update /tmp/leadflow-024a786b/lib/request-context.js getRequestId()
 *   to generate non-request synthetic IDs with crypto-backed entropy.
 * - Update /tmp/leadflow-024a786b/scripts/tasks/completion-reports-retention.js moveToArchive()
 *   collision suffix generation to use crypto-backed entropy.
 * - Update targeted tests in tests/unit/request-context.test.js and tests/unit/logger-requestcontext-integration.test.js.
 * Verify:
 * - node tests/unit/request-context.test.js exits 0.
 * - npx jest tests/unit/logger-requestcontext-integration.test.js --no-coverage exits 0.
 * - npm run lint exits 0.
 * - npm test exits 0.
 * - npm run build exits 0.
 * - npm audit --audit-level=high exits 0.
 * - grep -n "Math.random" lib/logger.js lib/request-context.js scripts/tasks/completion-reports-retention.js returns no executable-code matches.
 * Boundaries:
 * - Do not change route/controller behavior, DB schema, or service business logic.
 * - Do not change API contracts beyond stronger fallback ID/suffix entropy.
...
function requestLogger(req, res, next) {
  ...
  const startTime = Date.now()
  const requestId = req.headers['x-request-id'] || `req-${Date.now()}-${crypto.randomBytes(8).toString('hex')}`
  ...
  // Thread request context through async stack
