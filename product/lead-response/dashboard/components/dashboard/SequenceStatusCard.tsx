...
const STATUS_STYLES: Record<SequenceStatus, string> = {
  ...
  paused_by_agent: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400',
...
function SequenceRow({ sequence }: SequenceRowProps) {
  ...
  async function handleToggle() {
    ...
    // Both 'paused' and 'paused_by_agent' map to the resume endpoint
    try {
  ...
  const canToggle = status === 'active' || status === 'paused' || status === 'paused_by_agent'
...
export function SequenceStatusCard({ sequences, leadId }: SequenceStatusCardProps) {
  ...
  const visible = sequences.filter(
    (s) => s.status === 'active' || s.status === 'paused' || s.status === 'paused_by_agent'
  )
