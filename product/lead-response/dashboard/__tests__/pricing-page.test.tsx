...
describe('Team Tier ($399/mo)', () => {
  ...
  it('includes team-specific features', () => {
    const features = teamPlan?.features || []
    expect(features.some(f => f.toLowerCase().includes('team'))).toBe(true)
    expect(features.some(f => f.toLowerCase().includes('5 agent'))).toBe(true)
  })
...
describe('Grid Layout Responsiveness', () => {
  it('should use md:grid-cols-2 for 4 tiers on medium screens', () => {
    // This is a descriptive test - the actual grid class should be md:grid-cols-2 lg:grid-cols-4
