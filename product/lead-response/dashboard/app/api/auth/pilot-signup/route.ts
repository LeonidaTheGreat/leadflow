...
export async function POST(request: NextRequest) {
  try {
    ...
    // Send welcome email (non-blocking) — delegates to shared email-service for consistent pilot copy
    void sendWelcomeEmail(agent.email, agent.id, {
      agentName: `${agent.first_name} ${agent.last_name}`.trim(),
      planTier: 'pilot'
    }).catch((err: unknown) => { logger.error('[pilot-signup] Welcome email failed:', err) })
