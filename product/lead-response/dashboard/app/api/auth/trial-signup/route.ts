...
export async function POST(request: NextRequest) {
  try {
    ...
    // Calculate trial end date (14 days from now per PRD - frictionless onboarding)
    ...
    await logAnalyticsEvent('trial_signup_completed', agent.id, {
      source: 'trial_cta',
      utm_source: utm_source || null,
      utm_medium: utm_medium || null,
      utm_campaign: utm_campaign || null,
    ...
    const response = NextResponse.json({
      success: true,
      agentId: agent.id,
      redirectTo: '/dashboard', // Redirect to dashboard with sample data
