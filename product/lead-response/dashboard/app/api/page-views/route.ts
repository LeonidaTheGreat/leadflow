...
function extractAuthInfo(request: NextRequest): { agentId: string | null; sessionId: string | null } {
  ...
  if (authHeader?.startsWith('Bearer ')) {
    ...
    try {
      ...
      return { agentId: payload.userId || payload.id || null, sessionId: payload.sessionId || null }
    } catch {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const insertPayload: Record<string, string> = { agent_id: agentId, session_id: sessionId, page }
