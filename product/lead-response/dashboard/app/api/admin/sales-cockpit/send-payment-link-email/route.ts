 ...
 * Intended for the 3 onboarding-completed agents who are ready to pay.
 */
export async function POST(request: NextRequest) {
  ...
  if (!starterPriceId) {
    return NextResponse.json({ error: 'Starter plan price ID not configured (STRIPE_PRICE_STARTER_MONTHLY)' }, { status: 503 })
  }
  ...
  try {
    ...
    const { data: agent, error: agentError } = await postgrestAdmin
      ...
      .select('id,first_name,email,onboarding_completed')
      .eq('id', agentId)
      .single()
