...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data, error } = await postgrestAdmin
      ...
      .update(patch)
      .eq('id', id)
      .select('id,name,email,phone,brokerage_name,team_name,monthly_leads,current_crm,status,source,utm_source,utm_medium,utm_campaign,utm_content,utm_term,follow_up_sent,created_at,updated_at,contacted_at')
      .single()
