...
function PilotModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      trackEvent('form_open', { form_name: 'pilot_signup' })
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!validateForm()) {
      trackEvent('form_error', { form_name: 'pilot_signup', error_type: 'validation' })
      return
    }
    ...
    let utmParams = {}
    try {
      const utmData = sessionStorage.getItem('leadflow_utm')
      if (utmData) {
        utmParams = JSON.parse(utmData)
      }
    } catch {
      // ignore
    }
    ...
    try {
      const response = await fetch('/api/pilot-signup', {
        ...
        body: JSON.stringify({ ...formData, source: 'landing_page', utm: utmParams })
      })
      const result = await response.json()
      if (response.ok && result.success) {
        setIsSuccess(true)
        trackEvent('form_submit', { form_name: 'pilot_signup', status: 'success' })
        trackEvent('conversion', { type: 'pilot_signup', value: 1 })
      } else {
        alert(result.error || 'Something went wrong. Please try again.')
        trackEvent('form_error', { form_name: 'pilot_signup', error_type: 'api' })
      }
    } catch {
      alert('Network error. Please check your connection and try again.')
      trackEvent('form_error', { form_name: 'pilot_signup', error_type: 'network' })
    } finally {
...
export default function LandingPage() {
  ...
  const handleJoinPilotClick = () => {
    trackEvent('cta_click', { cta_name: 'join_pilot', location: 'hero' })
    setIsModalOpen(true)
  }
  ...
  const handlePricingCTAClick = () => {
    trackEvent('cta_click', { cta_name: 'join_pilot', location: 'pricing' })
    setIsModalOpen(true)
  }

  const handleFooterCTAClick = () => {
    trackEvent('cta_click', { cta_name: 'join_pilot', location: 'footer' })
    setIsModalOpen(true)
  }
    ...
    <div className="min-h-screen bg-white">
      ...
      <nav className={`sticky top-0 z-40 bg-white border-b border-gray-200 px-4 sm:px-6 py-4 transition-shadow ${scrolled ? 'shadow-md' : ''}`}>
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={handleJoinPilotClick} className="px-5 py-2.5 bg-teal-600 hover:bg-teal-700 text-white font-semibold rounded-md text-sm">Join Pilot</button>
          </div>
        </div>
      </nav>
      <section className="relative bg-gradient-to-br from-slate-800 to-slate-700 text-white py-20 sm:py-28 px-4 sm:px-6 overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto text-center relative z-10">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
            <button onClick={handleJoinPilotClick} className="px-8 py-4 bg-teal-600 hover:bg-teal-700 text-white font-semibold rounded-md">Join the Pilot Program (Free for 30 Days)</button>
            <button onClick={handleSeeHowItWorksClick} className="px-8 py-4 bg-white text-slate-800 border-2 border-white hover:bg-transparent hover:text-white font-semibold rounded-md">See How It Works</button>
          </div>
      ...
      <section className="py-20 px-4 sm:px-6 bg-white">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
            <div className="text-center p-6"><div className="text-4xl sm:text-5xl font-bold text-teal-600 mb-2">21x</div><div className="text-sm text-gray-600">More likely to convert in 5 min vs 30 min</div></div>
            <div className="text-center p-6"><div className="text-4xl sm:text-5xl font-bold text-teal-600 mb-2">&lt; 30 sec</div><div className="text-sm text-gray-600">LeadFlow AI response time</div></div>
            <div className="text-center p-6"><div className="text-4xl sm:text-5xl font-bold text-teal-600 mb-2">40%</div><div className="text-sm text-gray-600">More appointments for pilots</div></div>
            <div className="text-center p-6"><div className="text-4xl sm:text-5xl font-bold text-teal-600 mb-2">24/7</div><div className="text-sm text-gray-600">Coverage while you sleep</div></div>
          </div>
      ...
      <section id="pricing" className="py-20 px-4 sm:px-6 bg-white">
        <div className="max-w-lg mx-auto">
          ...
          <div className="bg-gray-50 border-2 border-teal-600 rounded-lg p-8 sm:p-12 text-center">
            ...
            <button onClick={handlePricingCTAClick} className="w-full py-4 bg-teal-600 hover:bg-teal-700 text-white font-semibold rounded-md mb-4">Join the Pilot Program</button>
      ...
      <section className="bg-gradient-to-br from-slate-800 to-slate-700 text-white py-20 px-4 sm:px-6 text-center">
        <div className="max-w-2xl mx-auto">
          ...
          <button onClick={handleFooterCTAClick} className="px-8 py-4 bg-teal-600 hover:bg-teal-700 text-white font-semibold rounded-md">Join the Pilot Program - Free for 30 Days</button>
