-- Migration: Pilot Recruitment Campaign Tracking
...
CREATE TABLE pilot_recruitment_campaigns (
...
CREATE TABLE pilot_recruitment_targets (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE pilot_recruitment_touchpoints (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_campaigns_status ON pilot_recruitment_campaigns(status);
CREATE INDEX idx_targets_campaign ON pilot_recruitment_targets(campaign_id);
CREATE INDEX idx_targets_status ON pilot_recruitment_targets(status);
CREATE INDEX idx_targets_channel ON pilot_recruitment_targets(source_channel);
CREATE INDEX idx_touchpoints_target ON pilot_recruitment_touchpoints(target_id);
CREATE INDEX idx_touchpoints_channel ON pilot_recruitment_touchpoints(channel);
...
-- Insert initial campaign for 30 pilots in 30 days
...
-- Insert seed targets based on existing research
