-- Migration 009: Extend pilot conversion email sequence with Day 75, 79, 85 milestones
-- Closes the 35-day gap between Day 55 and Day 90 expiry (the "Day 79 of 90" scenario)

-- Drop and recreate CHECK constraint to include all new milestones
...
-- Update the get_pilot_agents_for_milestone function to support new milestones
...
COMMENT ON FUNCTION get_pilot_agents_for_milestone IS 'Get pilot agents eligible for a specific conversion milestone (day_30, day_45, day_55, day_75, day_79, day_85)';

-- Update the view to include day_75, day_79, day_85 status columns
...
SELECT
    ...
    COALESCE(d75.status, 'pending') as day_75_status,
    d75.sent_at as day_75_sent_at,
    COALESCE(d79.status, 'pending') as day_79_status,
    d79.sent_at as day_79_sent_at,
    COALESCE(d85.status, 'pending') as day_85_status,
    d85.sent_at as day_85_sent_at,
...
GROUP BY a.id, a.email, a.name, a.plan_tier, a.pilot_started_at,
         d30.status, d30.sent_at, d45.status, d45.sent_at,
         d55.status, d55.sent_at, d75.status, d75.sent_at,
         d79.status, d79.sent_at, d85.status, d85.sent_at;
